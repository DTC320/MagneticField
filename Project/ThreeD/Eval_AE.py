...
def eval_ae(ae_model, test_loader):
    ae_model.eval()
    ...
    for batch_idx, (data,) in enumerate(test_loader):
        ...
        with torch.no_grad():
            recon_batch = ae_model(data)
            z = ae_model.encoder(data)

            recon_loss = recon_loss_fn(recon_batch, data).mean(dim=[1, 2, 3, 4]).numpy()
            latent_vectors.append(z.view(z.size(0), -1).cpu().numpy())
            labels.append(np.full((len(data),), batch_idx))

        if batch_idx < 10:
            for idx in range(len(data)):
                original_img = data[idx].cpu().numpy().transpose(1, 2, 3, 0)
                recon_img = recon_batch[idx].cpu().numpy().transpose(1, 2, 3, 0)
